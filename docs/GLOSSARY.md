...
### macOS
Apple's desktop operating system. JamfReports primarily targets macOS 14
(Sonoma) and macOS 15 (Sequoia); macOS 16 (Tahoe) support is in active
development.
...
several dashboards; once `jamf-cli pro sg` ships, "create smart group"
buttons will route through it.
...
54,000 rows is common on mid-sized fleets. Lives on the cold tier (24h).
...
### Cold tier
The 24-hour refresh tier in the HOT/WARM/COLD model. Holds the four
per-device commands: `ea-results --all`, `patch-status --scan-failures`,
`update-status --scan-failures`, `device-compliance`. Settings → Data &
Charts → Skip expensive collections suppresses these on manual GUI
refreshes; scheduled LaunchAgent collects always run them. *see also:
Refresh tier.*
...
### Dashboard
A tab in the app sidebar. 24 dashboards across POSTURE / OPERATIONS /
FLEET / AUTOMATION / CONFIGURATION groups + core tabs. Each toggleable in
Settings → Sidebar Visibility (except the four core tabs).

### Hot tier
The 15-minute refresh tier. Holds the `overview` indicator; refresh is
cheap (one API call). The user-perceived "live" surface.

### LaunchAgent
A macOS user-scoped scheduled job at `~/Library/LaunchAgents/com.jamfreports.*.plist`.
...
### Refresh tier
The HOT / WARM / COLD tier model in `CollectionTier`. Different commands
have different poll intervals based on API cost and data churn rate.
Defaults: hot = 15min, warm = 4h, cold = 24h.
...
### Risk Score
A per-device 14-factor weighted score producing a Critical / High / Medium
/ Low / Clean band. Lifted from `jamf_reports_cli_v3.5.py:
_compute_device_risk()`. Factors configurable in Config → Scoring. *see
...
### Warm tier
The 4-hour refresh tier. Holds policy-status, profile-status, and
similarly-priced calls. Indicator command for staleness probes is
`policy-status`.
...
### Sidebar
The 24-tab navigation rail. Groups: POSTURE, OPERATIONS, FLEET,
AUTOMATION, CONFIGURATION. Core tabs (Overview, Devices, Sources,
Settings) are pinned and cannot be hidden.
...
- `CLAUDE.md` — code-level conventions and invariants.
- `docs/INDEX.md` — top-level documentation map.
- `docs/onboarding/GETTING_STARTED.md` — first-run walkthrough including
  the Tab Reference for every dashboard.
