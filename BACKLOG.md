...
- **CONSIDER (closed by PR-22) — `snapshot-only` currently runs the full collect (20+ commands), making it expensive on on-prem fleets despite the mode's "Refresh data only" framing.** PR-20 emits the Trends summary at end of collect; PR-21 narrowed the mode contract but left the command set intact. PR-22 narrows `snapshot-only` to the Refresh tier (~5 commands) so it becomes safe to run once or twice daily without filling the workspace with timestamped snapshots. Surfaced because users may already be feeling the pain post-PR-20 (snapshot-only now actually feeds Trends, but each run is still heavy on the per-report total).
