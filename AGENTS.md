...
| `CLIBridge` / `CLIBridge+Run` | `Process`-based async wrapper for `jamf-cli` and `ReportEngine`. Streams stdout/stderr live to the Runs screen. All report generation uses the native Swift engine; no Python subprocess calls. `runNow(profile:mode:)` implements the Schedule-mode contract below, but has **no production callers** (2.6.1 audit) — the GUI "Run now" button goes through `LaunchAgentWriter.runNow`, which kickstarts the real LaunchAgent into the `--scheduled-run` path. Treat it as a tested reference implementation, not the live dispatcher. `explainExit(_:operation:)` (nonisolated static) maps a jamf-cli exit code to a plain-language cause + remediation (3→re-auth/401, 5→privileges/403, 6→throttled/429, 4→404, 1→network/per-command); used by every view that surfaces a non-zero exit instead of a bare number. |
...
| `LaunchAgentWriter` | Generates LaunchAgent plists and writes them atomically. **2.7.0:** a run-log write failure (e.g. the log directory is on a sync provider that evicted the file, or a share that went offline) is now logged once per label — matching `ScheduledRunRecorder.appendOrDrop`'s discipline — instead of being silently swallowed. |
...
| `ManagedAutomation` | (v2.2.0) Owns the global "managed" all-profiles LaunchAgents derived from `AutomationPolicy`. `reconcile` is declarative and scoped to a **reserved EXACT label set** (`managed-freshness`/`-scan`/`-reports`/`-backup` → `<prefix>.multi.managed-*`); `owns(_:)` is exact membership, NEVER a prefix match, so a user's hand-built multi-schedule can't be removed. Pure `desiredSchedules`/`plan`/`owns` are unit-tested; the thin `reconcile` executes through injectable install/remove closures (no real `launchctl` in tests). Idempotent (signature-skips unchanged — the signature is executable-path|mode|cadence-string|tiers|exclusions, and `LaunchAgentService.parse` reads `--exclude-profiles` and `ProgramArguments[0]` back off the plist, so an exclusions-only policy edit or a bundle that moved is detected rather than silently dropped — an agent written by a build-folder copy is repointed the first time the installed copy runs; `bundleLocationWarning` feeds the Automation screen banner and the Config Doctor `automation.bundle-location` row when the running bundle is outside /Applications or ~/Applications), force-reinstalls, and tears agents down when `isManaged` flips off. Called from `WorkspaceStore.reconcileManagedAutomation` on app launch (no-op in demo / when unmanaged) AND from the headless `--scheduled-run` path (`reconcileManagedAutomationHeadless` in `main.swift`, once per process, self-heals a policy edit or the RunAtLoad migration on hosts that never open the GUI); `reconcile(currentLabel:)` NEVER `launchctl bootout`s the label of the currently-running scheduled-run process — that action is routed through `installFileOnly`/`removeFileOnly` (plist write only; launchd re-reads the schedule at its NEXT load). |
...
| `AutomationHealth` | (2.6) Dead-man switch for scheduled runs (WorkspaceStore+Automation.swift). Pure `evaluate(inputs:now:)` turns per-agent `LaunchAgentService.ScheduleHealthInput` (expected fire from the new backward-looking `lastScheduledFireDate`, which steps the proven forward `nextDate` matcher across a capped 35-day window; last run from `<label>_status.json` — for `multi`/all-profiles agents without `--status-file`, the reader falls back to the newest per-profile `<workspace>/automation/<label>_status.json` the all-profiles run records, so managed agents never read as permanently overdue) into `.overdue` / `.failing` issues with a 60-minute grace. `AutomationHealthModel.shared` (@MainActor @Observable) publishes issues computed after `reconcileManagedAutomation` at launch and re-evaluated on `willBecomeActive`; OverviewView shows a summary banner ("Open Automation" action), AutomationView shows an Automation Health section. The once-daily overdue webhook digest posts from BOTH the GUI and the headless scheduled-run path (`main.swift` checks sibling schedules once per process), sharing the on-disk `DayMarker(name: "overdue-notify")` (`<workspace>/automation/.overdue-notify-last`), stamped only after a successful send so a failed delivery retries. |
