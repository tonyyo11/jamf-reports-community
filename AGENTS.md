...
Trends advanced. `StateFileStore`'s `.fail` files carry the last exit code as
...
| `TickScheduler` / `TickState` / `TickLock` | (2.8.0) `TickScheduler.due(schedules:lastStarted:runNowLabels:now:)` is the pure "what should run now" decision covering managed, hand-built, and legacy-imported schedules alike: a schedule is due when its most recent calendar fire (the same backward-stepping matcher the dead-man switch uses, see `LaunchAgentService` above) is later than its last recorded start, gated by the `runsAtLoad` catch-up rule (`snapshot-only`/`jamf-cli-full`/`csv-assisted` catch up from any missed fire; `jamf-cli-only`/`backup` only within the last 15 minutes) — plus any label with a run-now marker. `TickState` persists per-label "last started" stamps at `tick-state.json` (the spec called for the status file, which has no start time). `TickLock` is a pid file at `.tick.lock`: a live holder blocks a new tick outright, a dead or garbage holder is taken over, so a 300-second wake never piles a second run on top of a long collect. |
