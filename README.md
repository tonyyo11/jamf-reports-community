...
- **Period reports** — a workbook covering a quarter, a month, or any window you pick:
  a start figure, an end figure and the change for each metric, each carrying the date the
  data actually came from. See
  [Period Reports](https://github.com/tonyyo11/jamf-reports-community/wiki/06c-Period-Reports).
- **Shared workspaces** — point several Macs at one team folder (OneDrive/SharePoint, Box,
  Dropbox, Google Drive, a mounted share) and they build one pooled history, standing down
  for each other instead of collecting twice. See
...
- **Health strip** — a banner on every screen naming data sources that have stopped
  collecting or fallen behind, with a Collect now action; it also re-collects behind
  sources on its own while the app is open. See
...
- **Included command-line tool** — the same app binary is also a `jamf-reports` CLI for
  scripting reports, collection, backups, and diagnostics, with `check --json` to gate a
  CI job or monitoring probe on config health. Install it from Settings.
- **Readable backup diffs** — comparing two configuration backups summarizes what actually
  changed, collapsing objects that took the same update into one line, with the full
...
The same binary is also a `jamf-reports` command-line tool — run a recognized subcommand to
execute headlessly, or launch with no arguments for the GUI. Install the `jamf-reports`
command from **Settings → Command-line tool** (no administrator rights required), then:
