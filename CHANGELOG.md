...
- **`EmptyStateView`** (macOS app, shared component at `Theme/EmptyStateView.swift`): generalizes the Compliance Posture screen's empty-state pattern (optional icon + title + message + optional primary action). Retrofitted Compliance Posture (canonical usage) and Devices view. Includes accessibility hooks and `#Preview` variants for design verification.
