...
- **Accessibility — respect macOS system settings** (macOS app):
  ...
  - *Color-blind redundant encoding*: Protect severity Pills, Protect status Pills, Protect alerts-by-severity bars, Policy/Profile findings Pills, Compliance Posture controlBar labels, Patch compliance numerics, and Updates statusBar labels now pair color with an SF Symbol (`exclamationmark.triangle.fill` for critical, `exclamationmark.circle.fill` for high, `info.circle.fill` for medium, `circle.fill` for low). Severity remains identifiable for users with deuteranopia or under macOS *Display → Color Filters → Deuteranopia*.
  ...
  - *Dynamic Type for body text*: pinned `.system(size: ...)` calls for non-display body text were switched to SwiftUI semantic fonts (`.callout` / `.footnote` / `.caption` / `.caption.monospaced()`) so users on *System Settings → Displays → Larger Text* see body text, captions, and help text grow. The typographic identity (serif H1s, mono kickers, 32 pt numeric metrics) stays anchored. First-pass scope covers StatTile, FieldLabel, FieldHelp, SectionHeader trailingValue, EmptyStateView, AuditView, BackupsView, CompliancePostureView, ConfigView, DevicesView — fine per-screen detail across the remaining views is tracked as a follow-up.
