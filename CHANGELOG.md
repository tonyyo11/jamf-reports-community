...
- Tracked jamf-cli dependency updated to v1.17.0. No code changes required.
  ...
  - v1.17.0: New `--compact` flag (token-efficient output), `--select` flag (multi-field
    ...
    and VPP scope support. Destructive commands now annotated with `jamf:destructive`.
    None of these changes affect commands or flags used by jamf-reports-community.
