...
struct HealthCheckView: View {
    ...
    var body: some View {
        PageScaffold {
            ...
            HStack(spacing: 16) {
                SegmentedControl(
                    selection: Binding(
                        get: { tabKey(selectedTab) },
                        set: { selectedTab = tabIndex($0) }
                    ),
                    options: [
                ...
                .accessibilityLabel("View: \(tabTitle(selectedTab))")
                .help("Switch between Health Check, Group Hygiene, and the Config Doctor")
    ...
    private var header: some View {
        PageHeader(
            ...
            subtitle: tabSubtitle(selectedTab),
            lastModified: headerLastModified
        ) {
            AnyView(headerTrailing)
        }
    }

    private var headerLastModified: Date? {
        switch selectedTab { case 0: lastAuditDate; case 1: lastHygieneDate; default: nil }
    }
    ...
    private func tabKey(_ index: Int) -> String {
    ...
    private func loadDoctorReport() {
        ...
        Task {
            ...
            // Back on the View's main actor — discard if the profile/tab moved on.
